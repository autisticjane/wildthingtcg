...
if(!$_SERVER['QUERY_STRING']) {
	...
	while($row=mysql_fetch_assoc($select)) {
		...
		<form method="post" action="forms_general.php?thanks">
		<input type="hidden" name="id" value="<?php echo $row[id]; ?>" />
		<input type="hidden" name="name" value="<?php echo $row[name]; ?>" />
		<input type="hidden" name="email" value="<?php echo $row[email]; ?>" />
		<input type="hidden" name="url" value="<?php echo $row[url]; ?>" />
		<table width="100%">
		<tr><td valign="top"><b>Message:</b></td><td><textarea name="message" rows="5" cols="40">Type your message here.</textarea></td></tr>
...
elseif($_SERVER['QUERY_STRING']=="thanks") {
	if (!isset($_POST['submit']) || $_SERVER['REQUEST_METHOD'] != "POST") {
		...
		if (preg_match($bots, $_SERVER['HTTP_USER_AGENT'])) {
			...
			$message = "The following member has sent you a message: \n";
			...
			if (mail($recipient,$subject,$message,$headers)) {
				?>
				<h1>Thank You</h1>
				Thank you for sending in a contact form! I will try to get back to you within the next few days.
				If you don't here from me within a week, please resend the form.
				<?php
			}
			else {
				?>
